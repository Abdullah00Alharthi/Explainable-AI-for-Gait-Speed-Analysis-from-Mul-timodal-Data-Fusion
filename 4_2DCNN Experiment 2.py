...
# ===============================
# 3. Cross-Validation Setup (5 test subjects per fold)
# ===============================
n_folds = 5
test_subject_count = 5  # number of subjects for testing in each fold
seed = 42
np.random.seed(seed)
...
for fold in range(n_folds):
    ...
    # Randomly select 5 test subjects for this fold
    test_subjects = np.random.choice(all_subjects, size=test_subject_count, replace=False)
    remaining_subjects = np.setdiff1d(all_subjects, test_subjects)
    np.random.shuffle(remaining_subjects)
    
    # Split remaining subjects into validation and training
    n_val = max(1, len(remaining_subjects)//5)
    val_subjects = remaining_subjects[:n_val]
    train_subjects = remaining_subjects[n_val:]
    
    print(f"Test subjects for fold {fold+1}: {test_subjects}")
